...
:: External commands
if "%~1"=="status_nexusvpn" (
    call :test_service nexusvpn soft
    call :tcp_enable
    exit /b
)
...
sc query "nexusvpn" >nul 2>&1
if !errorlevel!==0 (
    for /f "tokens=2*" %%A in ('reg query "HKLM\System\CurrentControlSet\Services\nexusvpn" /v NEXUSVPN 2^>nul') do echo Service strategy installed from "%%B"
)

call :test_service nexusvpn
...
sc create %SRVCNAME% binPath= "\"%BIN_PATH%winws.exe\" !ARGS!" DisplayName= "NEXUSVPN" start= auto
sc description %SRVCNAME% "NEXUSVPN DPI bypass software"
sc start %SRVCNAME%
for %%F in ("!file%choice%!") do (
    set "filename=%%~nF"
)
reg add "HKLM\System\CurrentControlSet\Services\nexusvpn" /v NEXUSVPN /t REG_SZ /d "!filename!" /f
...
set "GITHUB_VERSION_URL=https://raw.githubusercontent.com/Flowseal/NEXUSVPN/main/.service/version.txt"
set "GITHUB_RELEASE_URL=https://github.com/Flowseal/NEXUSVPN/releases/tag/"
set "GITHUB_DOWNLOAD_URL=https://github.com/Flowseal/NEXUSVPN/releases/latest/download/NEXUSVPN-"
...
if not defined GITHUB_VERSION (
    echo Warning: failed to fetch the latest version. This warning does not affect the operation of NEXUSVPN
    timeout /T 9
    if "%1"=="soft" exit 
    goto menu
)
...
if !errorlevel!==0 (
    call :PrintRed "[X] Intel Connectivity Network Service found. It conflicts with NEXUSVPN"
...
if !winws_running! neq 0 if !windivert_running!==0 (
    ...
    if !errorlevel!==0 (
        ...
        set "conflicting_services=NEXUSVPN"
...
set "conflicting_services=NEXUSVPN discordfix_zapret winws1 winws2"
